...
async function main() {
  const port = Number(process.env.BASELINE_UI_PORT || 5179);
  ...
  try {
    ...
    await page.waitForSelector("button.dice-action", { timeout: 15_000 });
    await expect(await page.locator(".storyboard-card").count() === 0, "storyboards should not exist before story intent and storyboard generation");
    await expect(apiCalls.filter((call) => call.route === "video").length === 0, "video must not submit before package compile");

    await page.locator("button.dice-action").click();
    await waitForPagePredicate(page, "story intent render", () => document.body.textContent?.includes("baseline micro gag"));

    const storyboardButton = await waitForEnabledButtonMatching(page, "text => text.includes('生成候选') || text.includes('鍊欓€夊垎')", "storyboard generation must be enabled after story intent");
    await storyboardButton.click();
    await waitForPagePredicate(page, "storyboards render", () => document.querySelectorAll(".storyboard-card").length >= 3, 20_000);
    await expect(apiCalls.filter((call) => call.route === "video").length === 0, "video must not submit after storyboard generation alone");

    const packageButton = await waitForEnabledButtonMatching(page, "text => text.includes('编译') || text.includes('缂栬瘧')", "package compile must be enabled after storyboards");
    ...
    await waitForPagePredicate(page, "execution package render", () => document.body.textContent?.includes("FINAL VIDEO EXECUTION PACKAGE PROMPT") || document.body.textContent?.includes("baseline-video-task") || [...document.querySelectorAll("button")].some((button) => (button.textContent || "").includes("生成视频") || (button.textContent || "").includes("鐢熸垚瑙嗘")), 20_000);
    ...
    await expect(apiCalls.filter((call) => call.route === "story-intent").length === 1, "story intent must be called once");
    ...
    console.log("PASS UI flow: upload -> story intent -> storyboards -> execution package -> explicit video submit");
  } finally {
