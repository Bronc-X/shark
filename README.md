...
- History and post-production recovery: loaded history records with a saved local rendered MP4 must prefer `/api/local-video` over stale remote provider URLs. Subtitle and voiceover segments must be clamped to the loaded video duration before preview, voiceover generation, and final MP4 rendering.
- One-click post-production: "一键生成字幕和旁白" must generate per-segment voiceover, render the final subtitle/voiceover MP4, mark the current project as `完成`, keep the final local MP4 in the preview after refresh, and stay available as the repeat-regenerate control. There must not be a separate re-edit button.
- Generated-video parameter lock: after a raw or rendered video asset exists, aspect-ratio controls must be disabled. Clicking or force-triggering an aspect ratio after video generation must not clear `videoUrl`, `finalVideoPath`, the video preview, or the post-production panel.
- Left history summary: the left "最近 5 条" history panel is intentionally compact. Each row should show only the product name, current progress, and the four-step timeline; long file names, asset details, and expanded controls belong in the full history page.
